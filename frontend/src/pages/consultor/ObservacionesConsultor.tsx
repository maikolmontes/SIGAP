...
export default function ObservacionesConsultor() {
    ...
    const cargarObservaciones = useCallback(async () => {
        ...
        try {
            ...
            console.error('Error cargando observaciones consultor:', e);
        } finally {
            setLoading(false);
        }
    }, [filtroSemana]);
    ...
    return (
        <Layout rol="consultor" path="Auditoría / Observaciones">
            {/* Encabezado */}
            <div className="mb-6">
                <div className="flex items-center gap-3 mb-1">
                    <div className="w-10 h-10 rounded-2xl bg-gradient-to-br from-teal-500 to-blue-600 flex items-center justify-center shadow-md">
                        <MessageSquare className="w-5 h-5 text-white" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Observaciones de Supervisión</h1>
                        <p className="text-sm text-gray-500">
                            Auditoría de retroalimentaciones del Director en el Periodo: <span className="font-semibold text-teal-600">{periodoLabel}</span>
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-teal-400 focus:ring-1 focus:ring-teal-100"
                        />
                    </div>
                    <button
            ...
            {loading ? (
                ...
                <div className="text-center py-16 text-gray-400 bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <p className="font-medium">No se encontraron observaciones de supervisión</p>
                    <p className="text-sm mt-1">
                        {totalObs === 0
                            ? 'No se registran observaciones creadas por directores en este período.'
                            : 'Ajusta los filtros para visualizar más observaciones.'}
                    </p>
                </div>
            ) : (
                            ...
                            <div className="flex flex-col md:flex-row gap-4 justify-between">
                                <div className="flex-1">
                                    ...
                                    <div className="text-xs text-gray-500 flex flex-wrap gap-x-5 gap-y-1.5">
                                        ...
                                        {obs.director_nombre && (
                                            <p><strong className="text-gray-700">Registrada por:</strong> {obs.director_nombre}</p>
                                        )}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col justify-center items-end">
                                    <button
                                        onClick={() => navigate(`/consultor/agendas/${obs.id_usuario}`)}
                                        className="flex items-center gap-1.5 px-4 py-2 bg-teal-50 text-teal-700 text-xs font-bold rounded-xl hover:bg-teal-100 transition-colors border border-teal-100 whitespace-nowrap"
                                    >
