        ...
        <Layout rol="director" path="Supervisión / Historial de Agendas">
            ...
            <div className="mb-6">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Historial de Agendas</h1>
                        <p className="text-sm text-gray-500">
                            Periodo: <span className="font-semibold text-blue-600">{periodoLabel}</span>
                            {' · '}Ingeniería de Sistemas · {agendas.length} docentes
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                {[
                    { label: 'Total Docentes', value: agendas.length, icon: History, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
                    ...
                    { label: 'En Proceso', value: totalPendientes, icon: Clock, color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'border-yellow-100' },
                ].map(m => (
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
                <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Buscar docente o tipo de contrato..."
                            value={busqueda}
                                    ...
                                    <table className="w-full text-sm">
                                        <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wider border-b border-gray-100">
                                            <tr>
                                                ...
                                                <th className="px-5 py-3 text-left font-bold">Revisado por</th>
                                                ...
                                                return (
                                                    <tr key={a.id_usuario} className="hover:bg-blue-50/30 transition-colors">
                                                        ...
                                                        <td className="px-5 py-3.5 text-center">
                                                            <span className="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 rounded font-medium">{a.tipo_contrato}</span>
                                                        </td>
                                                        <td className="px-5 py-3.5 text-center text-sm font-semibold text-gray-700">
                                                            {Math.round(a.total_horas)}h
                                                        </td>
                                                        <td className="px-5 py-3.5 text-center">
                                                        ...
                                                        <td className="px-5 py-3.5 text-center text-sm text-gray-600">
                                                            {a.fecha_revision
                                                                ? new Date(a.fecha_revision).toLocaleDateString('es-CO')
                                                                : '—'}
                                                        </td>
                                                        <td className="px-5 py-3.5 text-sm text-gray-600">
                                                            {a.revisado_por_nombre ? (
                                                                <span className="text-xs text-indigo-700 font-medium">{a.revisado_por_nombre}</span>
                                                            ) : <span className="text-gray-400">—</span>}
                                                            {observacion && (
                                                                <div className="flex items-start gap-1 mt-1">
                                                                    <AlertTriangle className="w-3 h-3 text-orange-400 shrink-0 mt-0.5" />
                                                                    <p className="text-xs text-orange-600 line-clamp-2">{observacion}</p>
                                                                </div>
                                                            )}
                                                        </td>
                                                        <td className="px-5 py-3.5 text-center">
                                                            <button
                                                                onClick={() => navigate(`/director/agendas/${a.id_usuario}`, { state: { modo: 'historial' } })}
                                            ...
                                            {enRevision.map((a) => {
                                                ...
                                                return (
                                                    <tr key={a.id_usuario} className="hover:bg-yellow-50/30 transition-colors">
                                                        <td className="px-5 py-3.5">
                                                            <div className="flex items-center gap-2.5">
                                                                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-yellow-400 to-orange-500 flex items-center justify-center text-white font-bold text-xs shrink-0">
                                                                    {a.nombre_docente?.charAt(0)}{a.nombre_docente?.split(' ')[1]?.charAt(0) || ''}
                                                                </div>
                                                                <div>
                                                                    <p className="font-semibold text-gray-900 text-sm">{a.nombre_docente}</p>
                                                        ...
                                                        <td className="px-5 py-3.5 text-center">
                                                            {/* Estas agendas siguen pendientes: se abren en modo revisión
                                                                completo (aprobar, devolver y observaciones), no en consulta. */}
                                                            <button
                                                                onClick={() => navigate(`/director/agendas/${a.id_usuario}`, { state: { volverA: '/director/historial' } })}
