...
import { MessageSquare, Search, Eye, Filter, ChevronDown, Calendar, BookOpen, Clock, User, Crown, Building2 } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { useAuth } from '../../context/AuthContext';

export default function ObservacionesDecano() {
    const { user } = useAuth();
    ...
    useEffect(() => { cargarObservaciones(); }, [cargarObservaciones]);
    ...
    const periodoLabel = periodo ? `${periodo.anio}-${periodo.semestre === 1 ? 'I' : 'II'}` : 'Sin periodo activo';
    ...
    return (
        <Layout rol="decano" path={`Supervisión / Observaciones · ${facultadNombre}`}>
            <div className="mb-6">
                <div className="flex items-center gap-3 mb-1">
                    ...
                    <div>
                        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">Observaciones de Seguimiento</h1>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-6">
                <div className="px-5 py-4 border-b border-gray-100 flex flex-col sm:flex-row gap-3 items-start sm:items-center justify-between">
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input type="text" placeholder="Buscar docente, función o texto..."
                            value={busqueda} onChange={e => setBusqueda(e.target.value)}
                            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100" />
                    </div>
                ...
                {showFilters && (
                    <div className="px-5 py-3 bg-gray-50/50 border-b border-gray-100 flex flex-wrap gap-3">
                        ...
                        {(filtroSemana || filtroFuncion) && (
                            <button onClick={() => { setFiltroSemana(''); setFiltroFuncion(''); }}
            ...
            {loading ? (
                ...
                <div className="text-center py-16 text-gray-400 bg-white rounded-2xl shadow-sm border border-gray-100">
                    ...
                    <p className="text-sm mt-1">{totalObs === 0 ? 'Las observaciones se crean desde el detalle de agenda de cada docente.' : 'Ajusta los filtros para ver más resultados.'}</p>
                </div>
            ) : (
                <div className="grid grid-cols-1 gap-4">
                    {observacionesFiltradas.map((obs) => (
                        <div key={obs.id} className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md transition-all">
                            <div className="flex flex-col md:flex-row gap-4 justify-between">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-3 flex-wrap">
                                        <span className={`text-xs font-bold px-2.5 py-1 rounded-lg border ${obs.semana === 8 ? 'bg-blue-50 text-blue-700 border-blue-100' : 'bg-emerald-50 text-emerald-700 border-emerald-100'}`}>
                                        ...
                                        <span className="text-xs text-gray-400 flex items-center gap-1 ml-auto">
                                            ...
                                            {obs.ultima_edicion ? new Date(obs.ultima_edicion).toLocaleDateString('es-CO') : '—'}
                                        </span>
                                    </div>
                                    <p className="text-gray-800 text-sm mb-3 font-medium bg-gray-50 p-3 rounded-xl border border-gray-100 italic">"{obs.texto}"</p>
                                    <div className="text-xs text-gray-500 flex flex-wrap gap-x-5 gap-y-1.5">
                                        ...
                                        <p><strong className="text-gray-700">Actividad:</strong> {obs.rol_seleccionado}</p>
                                        {obs.director_nombre && <p><strong className="text-gray-700">Por:</strong> {obs.director_nombre}</p>}
                                    </div>
                                </div>
                                <div className="flex flex-col justify-center items-end">
                                    <button onClick={() => navigate(`/decano/agendas/${obs.id_usuario}`)}
                                        className="flex items-center gap-1.5 px-4 py-2 bg-emerald-50 text-emerald-700 text-xs font-bold rounded-xl hover:bg-emerald-100 transition-colors border border-emerald-100 whitespace-nowrap">
